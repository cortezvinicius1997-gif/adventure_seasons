...
public class GrassSeasonColors implements SimpleSynchronousResourceReloadListener
{
    ...
    public Identifier getFabricId() {
        return Identifier.fromNamespaceAndPath(AdventureSeasons.MODID, "grass_season_colors");
    }
    ...
    public void onResourceManagerReload(ResourceManager manager) {
        try{
            ...
            minecraftSwampGrass1 = new SeasonColor(JsonParser.parseReader(new InputStreamReader(swampGrass1.open(), StandardCharsets.UTF_8)));
            Resource swampGrass2 = manager.getResource(AdventureSeason.identifier("hardcoded/grass/swamp2.json")).orElseThrow();
            minecraftSwampGrass2 = new SeasonColor(JsonParser.parseReader(new InputStreamReader(swampGrass2.open(), StandardCharsets.UTF_8)));
        }catch (Exception e) {
            AdventureSeasons.LOGGER.error("[Adventure Mod] Failed to load hardcoded grass colors", e);
        }

        grassColorMap.clear();
        manager.listResources("seasons/grass", id -> id.getPath().endsWith(".json")).forEach((id, resource) -> {
        ...
        try {
            earlySpringColorMap = LegacyStuffWrapper.getPixels(manager, EARLY_SPRING_GRASS_COLORMAP);
            midSpringColorMap = LegacyStuffWrapper.getPixels(manager, MID_SPRING_GRASS_COLORMAP);
            lateSpringColorMap = LegacyStuffWrapper.getPixels(manager, LATE_SPRING_GRASS_COLORMAP);

            earlySummerColorMap = LegacyStuffWrapper.getPixels(manager, EARLY_SUMMER_GRASS_COLORMAP);
            midSummerColorMap = LegacyStuffWrapper.getPixels(manager, MID_SUMMER_GRASS_COLORMAP);
            lateSummerColorMap = LegacyStuffWrapper.getPixels(manager, LATE_SUMMER_GRASS_COLORMAP);

            earlyAutumnColorMap = LegacyStuffWrapper.getPixels(manager, EARLY_AUTUMN_GRASS_COLORMAP);
            midAutumnColorMap = LegacyStuffWrapper.getPixels(manager, MID_AUTUMN_GRASS_COLORMAP);
            lateAutumnColorMap = LegacyStuffWrapper.getPixels(manager, LATE_AUTUMN_GRASS_COLORMAP);

            earlyWinterColorMap = LegacyStuffWrapper.getPixels(manager, EARLY_WINTER_GRASS_COLORMAP);
            midWinterColorMap = LegacyStuffWrapper.getPixels(manager, MID_WINTER_GRASS_COLORMAP);
            lateWinterColorMap = LegacyStuffWrapper.getPixels(manager, LATE_WINTER_GRASS_COLORMAP);
